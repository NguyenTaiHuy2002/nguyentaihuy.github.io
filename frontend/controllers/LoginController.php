		...
		if (isset($_POST['submit'])) {
			...
			if (empty($this->error)) {
				...
				if (empty($user)) {
					...
					// Kiểm tra vai trò của người dùng (user role)
					if ($user['role'] === 'admin') {
						$this->success = 'Đăng nhập thành công với vai trò quản trị viên';
						$_SESSION['user'] = $user;
						header('Location: ../backend'); // Chuyển hướng tới trang quản trị
						exit();
					} elseif ($user['role'] === 'user') {
						$this->success = 'Đăng nhập thành công với vai trò người dùng';
						$_SESSION['user'] = $user;
